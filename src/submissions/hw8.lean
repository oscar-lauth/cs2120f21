...
begin
  ...
  unfold surjective,
  split,
  unfold total_function function defined single_valued,
  split,
...
def bij_trans (s : β → γ → Prop)  (r : α → β → Prop) :
  ...
  split,
  unfold composition,
  unfold function single_valued defined,
  assume a g1 g2 h1 h2,
  cases h1 with b1 h1pf,
  have rab1:=h1pf.elim_right,
  have sb1g1:=h1pf.elim_left,
  cases h2 with b2 h2pf,
  have rab2:=h2pf.elim_right,
  have b1eqb2:=r_fun rab1 rab2,
  have sb2g2:=h2pf.elim_left,
  apply s_fun sb1g1,
  rw b1eqb2,
  exact sb2g2,
  unfold defined composition, 
  assume a,
  have erab:= r_alldef a,
  cases erab with b rab,
  have esbg:=s_alldef b,
  cases esbg with g sbg,
  apply exists.intro g,
  apply exists.intro b (and.intro sbg rab),
  assume g,
  unfold composition,
  have esbg:= s_onto g,
  cases esbg with b sbg,
  have erab:= r_onto b,
  cases erab with a rab,
  ...
  apply exists.intro b (and.intro sbg rab),
  unfold composition,
  assume a1 a2 g h1 h2,
  cases h1 with b1 h1pf,
  have ra1b1:=h1pf.elim_right,
  have sb1g:=h1pf.elim_left,
  cases h2 with b2 h2pf,
  have ra2b2:=h2pf.elim_right,
  have sb2g:=h2pf.elim_left,
  have b1eqb2:= s_one_to_one sb1g sb2g,
  apply r_one_to_one ra1b1,
  rw b1eqb2,
  exact ra2b2,
  end
