import src/instructor/lectures/lecture_21,
...
/-
--each val must point to self as well as x could =y
...
--reflexive: every obj related to self
--anti-every object is not related to self
--less than is anti-refl
def irreflexive := anti_reflexive r -- sometimes used
def anti_symmetric := ∀ ⦃x y⦄, x ≺ y → y ≺ x → x = y
--less than or eq. if x ≤ y and y ≤ x, x must =y
def asymmetric := ∀ ⦃x y⦄, x ≺ y → ¬ y ≺ x
--if one is related to other then other is not related to first
--less than or greater then are asymmetric


--the transitive closure of a binary relation is the smallest relation that makes the relation transitive
