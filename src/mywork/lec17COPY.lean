...
def ods : set ℕ := { n : ℕ | odd n }
--are you allowed to write evens n ∨ ods n versus ev n ∨ odd n

def evens_union_ods : set ℕ := { n : ℕ |  evens n ∨ ods n }

def evens_intersect_ods : set ℕ  := { n : ℕ | evens n ∧ ods n }

def evens_complement : set ℕ := { n : ℕ | ¬evens n }

def ods_complement : set ℕ := { n : ℕ | ¬ods n }

def evens_intersect_empty : set ℕ := {n:ℕ | evens n ∧ empte n}

def evens_intersect_complete : set ℕ := {n:ℕ | evens n ∧ complete n}

def evens_union_empty : set ℕ := {n:ℕ | evens n ∨ empte n}

def evens_union_complete : set ℕ := {n:ℕ | evens n ∨ complete n}
...
s3=s1-s2, does s3 contain empty set still?
-/
#check evens \ ods
#check evens \ evens
#check evens \ empte
