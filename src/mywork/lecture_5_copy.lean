...
--def 
--proof is a value of type Proposition
/-
( P Q:Prop)
P ∧ Q
and.intro:
...
(which is to say we have a proof of each of them),
the proposition P ∧ Q is also true

Proof: The conjecture that P ∧ Q is true is proved by the application the intro rule for and
-/
example: 0=0 ∧ 1=1 :=
begin
  apply and.intro _ _, --conjecture
  apply eq.refl 0, --ingredient needed to prove conj.
  apply eq.refl 1, --ingredient needed to prove conj.
end
...
theorem and_communtative:∀ (P Q:Prop), P ∧ Q → Q ∧ P :=
begin
  assume P Q h,
  apply and.intro _ _,
