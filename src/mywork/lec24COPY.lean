...
Question: Is this symmetric? {(0,1), (1,0), (2,2)} --yes symmetric, 
for every relation/pair x,y, there is the pair y,x
How about this: {(0,1), (1,0), (2,3)}? --no, need (3,2)

Now suppose that we have a relation, r, over a set
of values, {0, 1, 2, 3, 4, 5}. Is this relation
reflexive? {} yes, for every element, must be related to itself
{(0,1),(2,3)}, not reflexive, nothing is self-related so irreflexive
...
Not necessarily reflexive. counterexample:  
-/
...
symmetric closure is union of orig and inverse of orig
-/

def reflexive_closure := λ (a b : β), (r a b) ∨ (a = b)
def symmetric_closure := λ (a b : β), (r a b) _ (r b a)
