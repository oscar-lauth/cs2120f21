...
relation is a subset of product set(product is like the big grid)
the subset of all pairs that satisy the predicate is the relation
elements of relation (which are pairs/tuples) must satisy 2 place predicate
relation is an element in the power set of a product set = relation is a subset of product set
...
example : nat.le 0 1 := 
begin
  apply nat.less_than_or_equal.step, --step proves that 0 ≤ 0, so we must fill in proof with refl below
  exact nat.less_than_or_equal.refl,
end
