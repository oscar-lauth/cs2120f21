/-
total function: domain of defintion = entire domain
single_valued means function (VLT)
single-valued: cannot be one-to-many
injective function: cannot be many to one, must pass VLT AND HLT, if x1 and x2 both map to y1, then x1 must=x2, are invertible
∀(a1,a2:α), r a1 b → r a2 b → a1=a2
surjective function-image of its domain of def. is its entire codomain, every B value in the codomain has been mapped from an A in the domain of def.
∀(b:β), ∃ a, r a b
bijective-one-to-one, inverse is bijective as well, for bi
-/
...
- y = log x, viewed as a function from ℝ → ℝ⁺, not defined for negative x values, if domain restricted to domain of def then it is surjective 
- y = x^2, viewed as a function from ℝ → ℝ, doesn't explore negative y values 
- y = x, viewed as a function from ℝ → ℝ, yes
- y = sin x, viewed as a function from ℝ → ℝ, no y values oscillate between -1,1
- y = sin x, as a function from ℝ to [-1,1] ∈ ℝ, surjective, codomain restricted to = range
-/
