...
--we want to prove the set {0,2} is a subset of evens
--consider each element in {0,2} by case analysis. Then show each element in {0,2} is in evens.
...
begin
  ...
  assume n,
  assume h,
  cases h, --cases only gives value of left or and right or returns membership in set
  --this is bc if you had like 3 elements in set. Left or is n=0, right is n∈{1,2}
  -- case: n = 0
  rw h,
...
begin
  ...
  --reversing implication to prove P using Q
  assume x,
  split,
  -- forward
  assume h,
