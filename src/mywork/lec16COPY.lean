...
set can't contain itself bc set of type T objects can only hold T objects can't hold an object of set T inside
no multiplicity in sets
object is either in set or isn't
membership: indication if value/element is in a set
subset: every element in subset is member of main set, subset can be the whole set
empty set: set of specific type so empty set from {1} is set nat
power set: set of all possible subsets, given {1,2,3} the power set contains 8(2^n) subsets (Type: set set nat)
10 elements in set, power set contains 2^10 sets
product set: set of all possible pairs from 2 sets. Set of n and set of m, product of these sets has size nxm,
product set has same type as 2 sets that formed it
{1,2,3,4}: display notation
{n : ℕ | n % 2= 0}: set defined by predicate, type of this prediate is nat → Prop
-/
